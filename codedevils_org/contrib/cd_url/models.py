...
class CustomUrl(models.Model):
    ...
    INTERVALS = (("HOURS", "HOURS"), ("DAYS", "DAYS"), ("MONTHS", "MONTHS"))
    ...
    slug = models.SlugField(
        ...
        max_length=20,
        help_text=_(
            "The string used to reference the URL from within a Django template. "
    ...
    last_updated = models.DateTimeField(
        ...
        help_text=_(
            "Used to send notifications to administrators when the link "
            "needs to be updated"
        ),
    )
    notify_in = models.PositiveSmallIntegerField(
        db_column="NotifyIn",
        blank=True,
        null=True,
        default=12,
        ...
        help_text=_(
            "The number of hours, days or months until managers are "
            "notified to update the link."
        ),
    )
    notify_interval = models.CharField(
        ...
        max_length=6,
        default="MONTHS",
        verbose_name=_("Notify interval"),
        help_text=_(
            "The interval (hours, days months) of ``notify_in``. Note that "
    ...
    acknowledged = models.BooleanField(
        ...
        help_text=_(
            "Used to flag if this link has been acknowledged as up-to-date. "
    ...
    def get_interval_in_hours(self):
        ...
        if self.notify_interval != "HOURS":
            hours = (
                self.notify_in * 24
                if self.notify_interval == "DAYS"
                else self.notify_in * 720
            )
            return hours
